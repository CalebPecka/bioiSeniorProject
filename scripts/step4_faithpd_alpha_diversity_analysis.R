...
alphaDiversity <- read.delim("../results/alpha-diversity.tsv", sep = "\t")
...
lm_alpha_with_age <- lm(faith_pd ~ age)
...
sig <- wilcox.test(C1$faith_pd, C2$faith_pd, paired = F)
...
ggplot(dat, aes(x = experimental_group, y = faith_pd)) +
  ...
  labs(title = paste0("Alpha Diversity Comparison in Week 0 Autism\n", 
       ...
       x = "Experimental Group", y = "Faith's Phylogenetic Diversity") +
...
lm_alpha_with_age <- lm(faith_pd ~ age + gender + weight..pounds., 
                        data_neurotypical)
summary(lm_alpha_with_age)
...
lm_alpha_with_age <- lm(faith_pd ~ age + weight..pounds., data_asd)
summary(lm_alpha_with_age)
