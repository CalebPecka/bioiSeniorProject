...
alphaDiversity <- read.delim("alpha-diversity-shannon.tsv", sep = "\t")
...
########################################
data_neurotypical <- data[which(data$experimental_group == "neurotypical" & data$week == 18),]

lm_alpha_with_age <- lm(shannon_entropy ~ age + gender + weight..pounds., data_neurotypical)
...
data_asd <- data[which(data$experimental_group == "autism" & data$week == 0),]

lm_alpha_with_age <- lm(shannon_entropy ~ age + weight..pounds. + end.of.treatment.PGI.R..improvement., data_asd)
summary(lm_alpha_with_age)
